...
fn create_mint_account(wallet_keypair: &Keypair, client: &RpcClient) -> Pubkey {
    ...
    if result.is_ok() {
        println!(
            "Successfully created a test Mint Account with Pubkey: {:?}",
            mint_account_pubkey
        )
    };
...
fn create_token_account(
    ...
    if result.is_ok() {
        println!(
            "Successfully created a Test Token Account with Pubkey: {:?}",
            account_mint_to_pubkey
        )
    };
...
fn mint_nft(
    ...
    let result = client.send_and_confirm_transaction_with_spinner(&transaction);
    if result.is_ok() {
        println!("Successfully Minted a test NFT to : {:?}", wallet_pubkey);
...
fn create_metadata_account(
    ...
    if result.is_ok() {
        println!(
            "Successfully created a new Test Metadata Account with Pubkey: {:?}",
            metadata_key
        )
    };
...
fn upgrade_to_master_edition(
    ...
    if result.is_ok() {
        println!("Upgraded Metadata Account to Master!");
    } else {
        println!("{:?}", result);
        return;
    }
